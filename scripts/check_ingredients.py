...
def load_ingredient_mapping():
    """Load the ingredient mapping from ingredients_mapping.json"""
    mapping_file = '/Users/max/Documents/15_Git-Repositories/MealPlanner_V3/data/ingredients_mapping.json'
...
def check_ingredients(interactive=False):
    """Check if all recipe ingredients are in the mapping
    
    Args:
        interactive: If True, prompt user for missing ingredients. If False, just return status.
    
    Returns:
        tuple: (success: bool, missing_ingredients: list)
    """
...
if __name__ == "__main__":
    check_ingredients(interactive=True)
